...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $heroSlides = Cache::remember("hero_slides_{$heroV}", 3600, fn () => HeroSlide::with('media')
        ...
        $categories = Cache::remember("home_all_categories_{$catV}_{$locale}", 3600, fn () => Category::with('media')
            ->whereNull('parent_id')
            ->orderBy('sort_order')
            ->get()
            ->map(fn ($c) => [
        ...
        $products = Cache::remember("home_featured_products_{$productV}_{$locale}", 3600, fn () => Product::with(['translations', 'variants', 'media'])
            ->where('is_active', true)
            ->orderBy('sort_order')
            ->take(8)
            ->get()
        ...
        $storeFeatures = Cache::remember("store_features_{$featV}", 3600, fn () => StoreFeature::orderBy('sort_order')
        ...
        return Inertia::render('home', [
            ...
            // shopSettings is provided globally by HandleInertiaRequests,
            // already filtered to the public whitelist — don't override it here.
            'products' => $products,
